...
%  MECHANICAL/THERMAL -- FIXED AT ONE END
p = 1;                   % Prescribed 
...
%%%
Fpp = barF(p); Fff = barF(f);

%% Newmark Family -- A Method

% TIME DATA
T    = 10;                  %%% Total Time
cmax = sqrt(E/rho);         %%% maximum expected velocity
...
%%%FV = zeros(tnn, tnts);
Val = 1e0;           % The magnitude of the applied force.
...
A(f,1) = Mff \ (Fff.*FV(f,1) - Kff*U(f,1));
